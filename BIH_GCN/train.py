...
def _run_one_rep(segments, labels, cfg: BIHGCNConfig, seed: int, device):
    ...
    X_tr_2d = X_tr.reshape(B, C * T)
    X_te_2d = X_te.reshape(len(te_idx), C * T)
    X_tr    = scaler.fit_transform(X_tr_2d).reshape(B, C, T)
    X_te    = scaler.transform(X_te_2d).reshape(len(te_idx), C, T)
...
def save_results(summary: dict, path: str, cfg: BIHGCNConfig):
    ...
    with open(path, "w") as f:
        ...
        f.write("\nPer-rep:\n")
        for i, (a, f) in enumerate(zip(summary["acc_all"], summary["f1_all"])):
            f.write(f"  rep {i+1:02d}  acc={a:.4f}  f1={f:.4f}\n")
