"""
Entry point for the WEAVE + SVM pipeline (v2).

Usage examples
--------------
# DEAP – Valence, single subject:
    python -m weave_pipeline_v2.run --dataset deap --data_path /path/to/deap --subject 1

# DEAP – Arousal, all subjects:
    python -m weave_pipeline_v2.run --dataset deap --data_path /path/to/deap --all_subjects --label_type A

# DEAP – Valence, all subjects, 32→16 channel reduction:
    python -m weave_pipeline_v2.run --dataset deap --data_path /path/to/deap --all_subjects --min_channels 16

# Emognition – single subject:
    python -m weave_pipeline_v2.run --dataset emognition --data_path /path/to/emog --subject 22

# Emognition – all subjects (recommended — more segments for stable estimates):
    python -m weave_pipeline_v2.run --dataset emognition --data_path /path/to/emog --all_subjects --min_channels 2
...
----------------
  ...
  5. Save detailed results to weave_logs_v2/
"""
...
from weave_pipeline_v2.config  import WEAVEConfig
from weave_pipeline_v2.dataset import load_data
from weave_pipeline_v2.train   import run_weave_evaluation, save_results
...
logger = logging.getLogger("WEAVE.Run.v2")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="WEAVE + SVM – EEG Emotion Recognition (v2)",
    ...
    # ── Output ────────────────────────────────────────────────────────────────
    p.add_argument("--log_dir",      default="weave_logs_v2/",
                   help="Directory for result files. Default=weave_logs_v2/")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("  WEAVE + SVM Pipeline  [v2]")
    logger.info("=" * 60)
